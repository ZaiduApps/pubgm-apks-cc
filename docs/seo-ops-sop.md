...
3. 将查询词分成品牌/导航、下载/安装、版本更新、故障排查、玩法攻略、社区/话题六类；采用“意图簇优先、页面覆盖最大化”策略，允许围绕一个版本或故障簇扩展多种标题、问法和长尾词，但每页仍需有可读正文和明确任务。
...
内容发布门槛（激进扩展模式）：内部确认的版本、活动和故障信息可以直接进入标题、摘要、正文、keywords、专题 SEO 配置和营销表达，不要求竞品或 Bing 已先出现；发布记录必须注明“内部确认”及确认日期，待公开来源出现后补链。允许使用强点击导向标题、同义词、错拼词、问句和场景化长尾词，但正文必须围绕对应意图提供步骤、条件、平台或版本说明。仍禁止伪造官方授权、伪造商店/下载安全承诺、虚构校验值或隐瞒已知限制；禁止隐藏文字、纯关键词门页、完全重复文章和只为 IndexNow 制造 URL。

### 激进关键词与版本策略（PUBGM）

- 版本词：内部确认版本可提前布局，例如 `PUBG Mobile 4.7`、`地铁逃生4.7版本`、`4.7更新内容`、`4.7下载安装`；标题和首段直接承接主词，页面正文补充当前已知内容与待确认项。
- 组合词：围绕下载、官网、APK、包名、登录、安装、更新、画质助手和电脑版组合扩展，不因竞品暂未覆盖而自动删除；同一页面允许覆盖多个近义词，避免为每个变体单独造薄页。
- 营销表达：可以使用“最新、完整、快速入口、一次看懂、避坑指南”等转化型措辞，但不得把第三方渠道写成官方渠道，不得承诺百分百成功、安全无风险或固定排名。
- 错拼和实验词：可写入非可见 keywords 或实验文章正文，单独标记实验，按 Bing 展现、点击和页面相关性复盘；一旦引发无关流量或质量信号下降，移除。
- 来源标记：文章 frontmatter/后台备注或运维记录中保存 `source_level=internal-confirmed|public-verified|editorial-hypothesis`；内部确认不等于已公开，搜索引擎提交和对外表述不得伪造第三方证明。
...
4. 仅对实际新增/更新/删除的 canonical URL 调用 IndexNow；激进扩展内容可以提高提交频率，但每次仍只提交真实存在、可访问且内容已完成的 URL，记录 URL、提交时间、响应状态、响应体摘要，随后分别等待并观察抓取/索引证据。
