...
export default function Home() {
  ...
  return (
    <div className="flex flex-col gap-12 md:gap-16 pb-16">
      ...
      <section
        id="home"
        className="relative w-full aspect-video flex items-center justify-center text-center text-white"
      >
        <Image
      ...
      {siteConfig.sections.map((section) => {
        ...
        return (
          <section
            key={section.id}
            id={section.id}
            className="container mx-auto px-4 md:px-6 scroll-mt-20"
          >
                ...
                {section.id === 'articles' ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                    {(section.items as Article[]).map((article) => (
                      <Card
                        key={article.slug}
                        className="group overflow-hidden flex flex-col h-full hover:shadow-lg transition-shadow duration-300"
                      >
                        <Link href={`/articles/${article.slug}`} className="flex flex-col h-full">
                          ...
                          <CardContent className="p-4 flex flex-col flex-grow">
                            <CardTitle className="text-base md:text-lg font-bold line-clamp-2 group-hover:text-primary transition-colors">
                              {article.title}
                            </CardTitle>
